...
describe("XferFactory", function () {
  ...
  it("Should transfer a token using the baseline function", async function () {
    ...
    const tx = await factory.baselineTransferFrom(token.address, RANDOM_ADDRESS, ethers.constants.WeiPerEther);
  ...
  it("Should transfer a token using address registry", async function () {
    ...
    const XferFactory = await ethers.getContractFactory("XferFactory");
    const factory = await XferFactory.deploy(registry.address);
    await factory.deployed();

    await registry.getId(RANDOM_ADDRESS);
    await registry.getId(token.address);
    ...
    const tx = await signer.sendTransaction({
      to: factory.address,
      data: ethers.utils.concat([
        padNumber(await registry.peekId(token.address), 2),
